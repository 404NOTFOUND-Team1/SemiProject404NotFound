...
public class EmailController {      //EmailController 클래스시작{
    ...
    public EmailController(EmailService service, PasswordEncoder passwordEncoder, MemberMapper mapper, UserInformation user, MemberService memberService) {     //EmailController2 클래스의 생성자선언, EmailService 타입의 emailService2를 받는다 //EmailService를 매개변수로 받아 컨트롤러에 주입 //이것은 클래스가 외부에서 EmailService의 객체를 필요로 하며, 이 객체는 외부에서 생성되어 주입됩니다
    ...
    public String checkEmail(@RequestParam String name, @RequestParam(required = false) String id, @RequestParam(required = false) String email, Model model) {
        System.out.println("2 pwd_key = " + name);
        System.out.println("2 num = " + id);
        ...
        if (userExists) {
            ...
            model.addAttribute("email", email);
            model.addAttribute("pwd_key", name);
//           model.addAttribute("email", (String)model.getAttribute("email"));
    ...
    @PostMapping("send-check2")
    @ResponseBody
    public String checkEmail2(@RequestBody String pwd_key, Model model) {
        System.out.println("3 pwd_key = " + pwd_key);
        boolean userExists = service.checkEmail2(pwd_key, user.getId());    //name 값과 id 값을 가져왔으니 db에 저장된 name와 id 값을 비교후 같을시
        //리턴으로 비밀번호 변경페이지로 이동
        if (userExists) {
            model.addAttribute("num", user.getId());
            model.addAttribute("pwd_key", pwd_key);
//           model.addAttribute("email", (String)model.getAttribute("email"));
            System.out.println("여기2");  //여기2까지 옴
            return "인증 성공";
        } else {
            model.addAttribute("error", pwd_key);
            return "인증 실패"; // 에러페이지 추가해서 적용
        }
    }
